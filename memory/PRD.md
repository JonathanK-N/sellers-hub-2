...
- **Push notifications (Firebase) — DONE ✅**: New `notifications_router.py` exposes the in-app feed (`GET /notifications`, `/unread-count`, `POST /{id}/read`, `/read-all`), device tokens (`POST`/`DELETE /notifications/device-token`), and per-type preferences (`GET`/`PATCH /notifications/preferences`). `notifications.py` `_send_fcm` modernized: prefers FCM HTTP v1 (FIREBASE_PROJECT_ID + FIREBASE_SA_JSON via google-auth), falls back to legacy FIREBASE_SERVER_KEY, then simulated logging in sandbox. Frontend: `lib/push.js` (permission request + token registration, Firebase-ready but simulated by default), `/notifications` page (feed, push toggle, per-type prefs), TopBar bell wired to unread count, silent token re-registration on login when permission granted, token deactivation on logout, `public/firebase-messaging-sw.js` service worker (inert without real config). Env to set for production: REACT_APP_FIREBASE_CONFIG + REACT_APP_FCM_VAPID_KEY (frontend), FIREBASE_PROJECT_ID + FIREBASE_SA_JSON (backend).
