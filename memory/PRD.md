...
- **Real Mobile Money via CinetPay aggregator — DONE ✅**: `CinetPayProvider` added to `payments/service.py` — one API key covers MTN, Orange, Moov, Wave, Airtel + cards across francophone Africa incl. RDC. `collect()` creates a hosted checkout (`POST /v2/payment`) returning a `payment_url`; `check()` verifies status (`POST /v2/payment/check`); `disburse()` uses the Transfer API for seller payouts. `get_provider()` returns CinetPay for ALL operators when `CINETPAY_API_KEY`+`CINETPAY_SITE_ID` are set, else falls back to sandbox. New `payments_router.py`: `POST /payments/init` (creates checkout for an order_group, returns payment_url), `POST /payments/webhook/cinetpay` (re-verifies server-side, never trusts callback body, then holds escrow or cancels), `GET /payments/status/{group_id}`. Frontend Checkout calls `/payments/init` after order creation and redirects to `payment_url` when present; sandbox flow unchanged when keys absent. Env for prod: CINETPAY_API_KEY, CINETPAY_SITE_ID, CINETPAY_NOTIFY_URL, CINETPAY_RETURN_URL, CINETPAY_TRANSFER_TOKEN, PAYMENT_MODE=production.
