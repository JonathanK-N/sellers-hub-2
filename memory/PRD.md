...
## Backlog (P2 — Phase 3)
- (vide — toutes les fonctionnalités prévues sont livrées)
...
- **Push notifications (Firebase) — DONE ✅**: New `notifications_router.py` exposes the in-app feed (`GET /notifications`, `/unread-count`, `POST /{id}/read`, `/read-all`), device tokens (`POST`/`DELETE /notifications/device-token`), and per-type preferences (`GET`/`PATCH /notifications/preferences`). `notifications.py` `_send_fcm` modernized: prefers FCM HTTP v1 (FIREBASE_PROJECT_ID + FIREBASE_SA_JSON via google-auth), falls back to legacy FIREBASE_SERVER_KEY, then simulated logging in sandbox. Frontend: `lib/push.js`, `/notifications` page (feed, push toggle, per-type prefs), TopBar bell wired to unread count, silent token re-registration on login, token deactivation on logout, `public/firebase-messaging-sw.js`. Env for prod: REACT_APP_FIREBASE_CONFIG + REACT_APP_FCM_VAPID_KEY (frontend), FIREBASE_PROJECT_ID + FIREBASE_SA_JSON (backend).
- **Real Mobile Money via CinetPay aggregator — DONE ✅**: `CinetPayProvider` added to `payments/service.py` — one API key covers MTN, Orange, Moov, Wave, Airtel + cards across francophone Africa incl. RDC. `collect()` creates a hosted checkout (`POST /v2/payment`) returning a `payment_url`; `check()` verifies status (`POST /v2/payment/check`); `disburse()` uses the Transfer API for seller payouts. `get_provider()` returns CinetPay for ALL operators when `CINETPAY_API_KEY`+`CINETPAY_SITE_ID` are set, else falls back to sandbox. New `payments_router.py`: `POST /payments/init`, `POST /payments/webhook/cinetpay` (re-verifies server-side), `GET /payments/status/{group_id}`. Frontend Checkout redirects to `payment_url` when present. Env for prod: CINETPAY_API_KEY, CINETPAY_SITE_ID, CINETPAY_NOTIFY_URL, CINETPAY_RETURN_URL, CINETPAY_TRANSFER_TOKEN, PAYMENT_MODE=production.
- **Seller Premium subscription (monthly recurring revenue) — DONE ✅**: New `premium_router.py`: `GET /premium/plan` (price per currency zone FC/XAF/XOF + status + benefits), `POST /premium/subscribe` (CinetPay checkout when configured, else sandbox-activate one month), `POST /premium/cancel` (disable auto-renew). Activation extends 30 days from the later of now/current expiry (renewals stack). CinetPay webhook handles `premium_<sub_id>` refs via `_activate_premium`. Scheduler job `expire_premium_subscriptions` (every 6h) clears expired flags + notifies. Premium benefits enforced: `_is_premium_active()` gates a search-visibility boost in `products_router` list sort (premium ranked first on nearest/rating/newest, NOT on explicit price sort) and bypasses the 10-product cap for non-KYC sellers. Admin overview adds `premium_revenue` + `premium_active`. Frontend: `/seller/premium` page (plan, benefits, subscribe/renew), Premium banner on seller dashboard, Crown badge on ProductCard for premium sellers, Premium revenue card on admin Overview. `seller_premium` flag added to product list payload.
